...
func main() {
	...
	for decision == 0 && nBoneyard >= 4 {
		...
		// apabila pilihan adalah 9 maka keluar dari loop
		if decision == 9 {
			break
		}
		...
		if decision == 0 {
			...
			fmt.Println("Dealer's tiles:")
			// cari pemenang dengan memanggil subprogram menang
			pemenang(&playerTiles, &dealerTiles, &playerScore, &dealerScore, &total)
...
func haveTwoBalak(p arrDomino) bool {
	...
	// KERJAKAN MANDIRI
	return haveBaak(p[0]) && haveBalak(p[1])
